...
class Ujian extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->mhs 	= $this->ujian->getIdpeserta($this->user->username);
	}

	public function akses_tim_soal()
	{
		if (!$this->ion_auth->in_group('Tim Soal')) {
			show_error('Halaman ini khusus untuk tim_soal untuk membuat Test Online, <a href="' . base_url('dashboard') . '">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
		}
	}
	...
	public function add()
	{
		...
		$data = [
			...
			'babak'	=> $this->soal->getbabaktim_soal($user->username),
			'tim_soal'		=> $this->ujian->getIdtim_soal($user->username),
		];
	...
	public function validasi()
	{
		...
		$user 	= $this->ion_auth->user()->row();
		$tim_soal 	= $this->ujian->getIdtim_soal($user->username);
		$tim_soal1 = $this->ujian->getkelasid($tim_soal->id_tim_soal);
		$jml 	= $this->ujian->getJumlahSoal($tim_soal1->kelas_id, $tim_soal1->babak_id)->jml_soal;
	...
	public function save()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($method === 'add') {
				$input['tim_soal_id']	= $tim_soal_id;
				$input['babak_id'] = $babak_id;
				$input['token']		= $token;
	...
	public function list_json()
	{
		...
		$list = $this->ujian->getListUjian($this->mhs->id_peserta, $this->mhs->kelas_id);
	...
	public function list()
	{
		...
		$data = [
			...
			'judul'		=> 'Olimpiade',
			'subjudul'	=> 'Soal',
			'mhs' 		=> $this->ujian->getIdpeserta($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function index()
	{
		...
		$mhs		= $this->mhs;
		$h_ujian 	= $this->ujian->HslUjian($id, $mhs->id_peserta);
		...
		if ($cek_sudah_ikut < 1) {
			...
			$waktu_selesai = $ujian->terlambat;
			$time_mulai		= date('Y-m-d H:i:s');

			$input = [
				'ujian_id' 		=> $id,
				'peserta_id'	=> $mhs->id_peserta,
		...
		if (!empty($soal_urut_ok)) {
			...
			foreach ($soal_urut_ok as $s) {
				...
				if ($ujian->babak_id == 9) {
					$html .= '<input required="required" name="opsi_' . $no . '" id="jawaban" class="form-control" style="width:100%!important" value="">
                            </input>';
				}
	...
	public function simpan_satu()
	{
		...
		$list = $this->ujian->getJawaban($id_tes);
		$pc_jawaban = explode("#", $list->list_jawaban);
		$jumlah_soal	= sizeof($pc_jawaban);

		foreach ($pc_jawaban as $jwb) {
			$pc_dt 		= explode("@", $jwb);
			$id_soal 	= $pc_dt[0];
			$key 	= $this->soal->getSoalById($id_soal);
			$key_jwb .= "" . $key->jawaban . "#";
		}
	...
	public function simpan_akhir()
	{
		...
		$kode = $this->ujian->getUjianById($list_jawaban->ujian_id);
		// Pecah Jawaban
		if ($kode->babak_id == 4) {
			$pc_jawaban = explode("#", $list_jawaban->list_jawaban);
			...
			$nilai = ($jumlah_benar * 4) + ($jumlah_salah * -1);
