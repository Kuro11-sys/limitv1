...
$(document).ready(function () {
    if (banyak < 1 || banyak > 50) {
        ...
        window.location.href = "tahun";
    } else {
        generate(banyak);
    }

    $('#inputs input:first').select();
    $('form#tahun input').on('change', function () {
        $(this).parent('.form-group').removeClass('has-error');
        $(this).next('.help-block').text('');
    });

    $('form#tahun').on('submit', function (e) {
        e.preventDefault();
            ...
            success: function (data) {
                ...
                if (data.status) {
                    Swal.fire({
                        ...
                        if (result.value) {
                            window.location.href = 'http://localhost/new/tahun';
                        }
                    });
                } else {
                    var j;
            ...
            <tr>
                ...
                <td>
                    <div class="form-group">
                        <input autocomplete="off" type="text" name="nama_tahun[${i}]" class="input-sm form-control">
