...
function cek_status_ragu(id_soal) {
  ...
  if (status_ragu == "N") {
    $(".ragu_ragu").html('<i class="fas fa-exclamation-triangle"></i> Ragu');
  } else {
    $(".ragu_ragu").html(
      '<i class="fas fa-exclamation-triangle"></i> Tidak Ragu'
    );
  }
}
      ...
      if (ragu == "Y") {
        if (jawab == "-") {
          hasil_jawaban +=
            '<a id="btn_soal_' +
            i +
            '" class="btn btn-default btn_soal btn-sm" onclick="return buka(' +
            i +
            ');">' +
            i +
            ". " +
